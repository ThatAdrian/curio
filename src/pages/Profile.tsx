...
export default function Profile() {
  ...
  async function load() {
    ...
    if (ids.length) {
      const { data: si } = await supabase.from("shelf_items").select("*, media_items(*)").in("shelf_id", ids);
      ...
      <div className="profile-head">
        ...
        <div className="profile-actions">
          {isOwn
            ...
            : session && <button className={"btn small" + (iFollow ? "" : " primary")} onClick={toggleFollow}>{iFollow ? "Following ✓" : "+ Follow"}</button>}
        </div>
      </div>
